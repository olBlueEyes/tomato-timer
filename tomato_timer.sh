...
# only print the date to log if it's different from today's
	...
	if [ "$(tail -1l $log)" != "$last_log_date" ]; then
		...
		# below is an alternate method with sed
		# read logsec log_minutes <<< $(tail -1 $drafts/tomato.log | sed 's/.*(\([0-9]\{2\}\):\([0-9]\{2\}\)).*/\1 \2/'
...
function tomato_trap {
	...
	# else, get the ending time and write the info to the log
		...
		# see tomato_end for details on these variables
		...
		echo -e "\n---------------\ntimer aborted with $minutes minutes and $seconds seconds left"

		# write the elapsed session to the log
		echo test
		echo -e "\t$start_time - $end_time ($elapsed_min:$elapsed_sec)\t$tomato_count" | tee -a $log
		exit
...
function tomato_time {
	...
	while [ $countdown -gt 0 ]; do
		echo "$(($countdown/60)) minutes left"
		countdown=$(($countdown-$message_rate))
		sleep $message_rate
		tomato_end
	done
}
...
function tomato_end {
	if [ $countdown == 0 ]; then
		...
		elapsed_min=$(printf "%02d" $((($end-$start)/60)))
		elapsed_sec=$(printf "%02d" $((($end-$start)%60)))

		total_hr=$(printf "%02d" $(($elapsed_min+$log_minutes)))
		total_min=$(printf "%02d" $((($elapsed_min+$log_minutes)/60+$log_hours)))

		# total_min=$( printf "%02d" $(($total_min+$elapsed_min)) )
		# total_hr=$( printf "%02d" $((10#$total_hr+10#$elapsed_sec)) )

		echo -e "\t$start_time - $end_time ($elapsed_min:$elapsed_sec)\t$total_min:$total_hr\t$tomato_count" | tee -a $log

		# as before, write the latest log totals to the vars
